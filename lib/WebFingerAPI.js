...
class WebFingerAPI {
    ...
    constructor(config) {
      ...
      this.statements = JSON.parse(fs.readFileSync('./etc/entityStatements.json', "utf8"))
    ...
    query(query) {
      return new Promise((resolve, reject) => {
        ...
        if (query.resource === this.config.get('issuer')) {
          links.push({
            "rel": this.fedrel,
            "href": this.baseurl + 'federation-api/entitystatements'
          })
        }
        ...
        if (query.rel === this.fedrel) {
          if (this.statements.hasOwnProperty(query.resource)) {
            links.push({
              "rel": this.fedrel,
              "href": this.baseurl + 'federation-api/entitystatements?entity=' + encodeURIComponent(query.resource)
            })
          }
        }
        return resolve({
    ...
    entitystatement(query) {
      return new Promise((resolve, reject) => {
        ...
        if (resource === this.config.get('issuer')) {
          ...
          statement.jwks = this.jwks.getJWT('verify', this.config.get('key'))
        } else {
